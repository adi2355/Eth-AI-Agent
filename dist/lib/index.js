        ...
        switch (command) {
            case 'info':
                ...
                if (process.env.SEPOLIA_RPC_URL)
                    console.log('- Sepolia');
                if (process.env.OPTIMISM_GOERLI_RPC_URL)
                    console.log('- Optimism Goerli');
                if (process.env.ARBITRUM_GOERLI_RPC_URL)
                    console.log('- Arbitrum Goerli');
                break;
            case 'connect':
                const provider = args[1] || 'metamask';
                console.log(`Connecting to wallet using ${provider}...`);
                const address = await exports.blockchainApi.connectWallet(provider);
                console.log(`Connected to wallet: ${address}`);
                break;
            case 'deploy':
                console.log('Deploying contract...');
                const result = await exports.blockchainApi.deployContract('ERC20', {
                    name: 'Demo Token',
                    symbol: 'DEMO',
            ...
            case 'transfer':
                ...
                const transferResult = await exports.blockchainApi.transferTokens('0x1234567890123456789012345678901234567890', '0.1');
